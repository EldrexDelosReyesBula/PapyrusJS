...
- [NPM Module vs CDN Global](#npm-module-vs-cdn-global)
...
## NPM Module vs CDN Global

When working on different environments (building standard websites vs complex apps using bundlers), you will interface with Papyr in one of two ways.

### CDN / Global Namespace
...
#### Code Usage
The library is attached directly to the global `window` object:
```javascript
// Access window.papyr directly
let count = papyr.state(0);
```
...
#### Installation
You install the dependency via npm:
```bash
npm i @eldrex/papyr
```

#### Code Usage
Import the package named export (`papyr`) or default export:
```javascript
// ES Module imports
...
Or for CommonJS:
```javascript
const { papyr } = require('@eldrex/papyr');
```
